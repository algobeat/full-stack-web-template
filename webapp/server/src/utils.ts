...
export function createJWT(email: string) {
  // 24 hour expiry.
  return jwt.sign({email, expires: Date.now() + 24 * 60 * 60 * 1000}, process.env.AUTH_SIGN_SECRET!);
}
...
export function renewJWT(email: string, res: express.Response) {
  ...
  res.cookie('authorization', token, {httpOnly: true, expires: payload?.expiresDate});
  return token;
}
